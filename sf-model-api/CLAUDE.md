...
**Type:** Python Flask-based API server with OpenAI-compatible endpoints
**Purpose:** Provides OpenAI-compatible API interface for Salesforce-hosted LLM models with advanced tool calling capabilities
...
- **API Server:** `llm_endpoint_server.py` - OpenAI-compatible endpoints
- **Tool Calling:** `tool_handler.py` - Advanced function calling framework
- **Token Management:** Thread-safe authentication with proactive refresh
- **CLI Interface:** `cli.py` - Direct model interaction and testing
- **Configuration:** `config.json` - Environment-based configuration
...
#### High-Impact Issues
1. **Token Management Overhead** (`llm_endpoint_server.py:44-582`)
 - Excessive file I/O operations on each request validation
 - Overly aggressive token refresh (3-minute daemon, 15-minute TTL)
 - Thread lock contention during token operations

2. **Response Processing Inefficiency** (`llm_endpoint_server.py:613-734`)
 - Multiple fallback response parsing paths (9 different attempts)
 - Inefficient string operations and repeated dictionary access
 - Large response handling without proper streaming

#### Medium-Impact Issues
3. **Tool Calling Performance** (`tool_handler.py:302-474`)
 - Complex regex processing for n8n parameter extraction
 - Unbounded conversation state accumulation
 - String manipulation overhead in conversation building

4. **Memory Management** (`tool_handler.py:47-84`)
 - No bounds on conversation history growth
 - Thread-local storage accumulation without cleanup
 - Token cache growth without eviction policies

5. **Network Latency** (`llm_endpoint_server.py:914-944`)
 - Synchronous API calls to Salesforce
 - Fixed timeouts without adaptive scaling
 - Inefficient retry logic with linear backoff
...
- 📋 Circuit breaker pattern for Salesforce API resilience
...
# Monitor token cache effectiveness
python -c "from llm_endpoint_server import get_cached_token_info; print('Cache analysis needed')"

# Memory profiling under load
python -m memory_profiler llm_endpoint_server.py
```

## ⚠️ Known Performance Pitfalls

### Critical Issues to Avoid
1. **Token Refresh Thrashing:** Avoid excessive token file I/O operations
2. **Memory Leaks:** Implement conversation state bounds and cleanup
3. **Thread Contention:** Minimize lock duration and use granular locking
...
**User Prompt:** "This project is a mess. Can you analyze my Express.js API, create documentation for it, and refactor the `user Controller.js` file to be more efficient?"
