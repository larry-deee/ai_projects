...
class ToolCallingConfig(BaseModel):
    """Configuration for tool calling behavior with enhanced security profiles."""
    ...
    enable_write_operations: bool = Field(
        default=False,
        description="Whether to allow write operations"
    )
    write_operation_whitelist: List[str] = Field(
        default_factory=list,
        description="List of specific write functions that are allowed"
    )
    allowed_dangerous_functions: List[str] = Field(
        default_factory=list,
        description="List of specific dangerous functions that are allowed"
    )
    blocked_functions: List[str] = Field(
        default_factory=list,
        description="List of specific functions that are explicitly blocked"
    )
    security_profile: str = Field(
        default="production",
        description="Security profile: development, staging, or production"
    )
...
SECURITY_PROFILES = {
    "development": {
        ...
        "write_operation_whitelist": ["Write", "MultiEdit", "EditFile", "CreateFile"],
        "allowed_dangerous_functions": ["write", "read", "create", "modify", "file", "edit"],
        "blocked_functions": [],
    ...
    "staging": {
        "allow_dangerous_functions": True,
        "enable_write_operations": True,
        "write_operation_whitelist": ["Write", "MultiEdit"],
        "allowed_dangerous_functions": ["write", "read", "file"],
        "blocked_functions": ["delete", "remove", "destroy"],
        "strict_parameter_validation": True,
    ...
    "production": {
        ...
        "write_operation_whitelist": [],
        "allowed_dangerous_functions": [],
        "blocked_functions": ["write", "edit", "create", "modify", "delete", "remove"],
        "strict_parameter_validation": True,
...
def load_tool_config_from_file(config_file: str = None) -> ToolCallingConfig:
    """
    Load tool configuration from config.json file with environment variable overrides.
    
    Args:
        config_file: Path to config file (None for discovery)
    
    Returns:
        ToolCallingConfig instance
    """
    config_data = {}
    ...
    if config_file and os.path.exists(config_file):
        try:
            with open(config_file, 'r') as f:
                ...
                config_data = full_config.get('tool_calling', {})
                logger.info(f"Loaded tool configuration from {config_file}")
        except Exception as e:
            logger.warning(f"Failed to load tool config from file: {e}")
    
    # Apply security profile if specified
    profile = config_data.get('security_profile') or os.getenv('SF_TOOL_SECURITY_PROFILE', 'production')
    if profile in SECURITY_PROFILES:
        base_config = SECURITY_PROFILES[profile].copy()
        base_config.update(config_data)  # Override with explicit settings
        config_data = base_config
        logger.info(f"Applied security profile: {profile}")
    
    # Environment variable overrides
    env_overrides = {}
    if os.getenv('SF_ALLOW_DANGEROUS_FUNCTIONS'):
        env_overrides['allow_dangerous_functions'] = os.getenv('SF_ALLOW_DANGEROUS_FUNCTIONS').lower() == 'true'
    if os.getenv('SF_ENABLE_WRITE_OPERATIONS'):
        env_overrides['enable_write_operations'] = os.getenv('SF_ENABLE_WRITE_OPERATIONS').lower() == 'true'
    if os.getenv('SF_STRICT_VALIDATION'):
        env_overrides['strict_parameter_validation'] = os.getenv('SF_STRICT_VALIDATION').lower() == 'true'
    if os.getenv('SF_WRITE_WHITELIST'):
        env_overrides['write_operation_whitelist'] = os.getenv('SF_WRITE_WHITELIST').split(',')
    if os.getenv('SF_ALLOWED_DANGEROUS'):
        env_overrides['allowed_dangerous_functions'] = os.getenv('SF_ALLOWED_DANGEROUS').split(',')
    if os.getenv('SF_BLOCKED_FUNCTIONS'):
        env_overrides['blocked_functions'] = os.getenv('SF_BLOCKED_FUNCTIONS').split(',')
    ...
    # Create and return ToolCallingConfig instance
    try:
        return ToolCallingConfig(**config_data)
    except Exception as e:
        logger.error(f"Failed to create ToolCallingConfig: {e}, using defaults")
...
def repair_malformed_json(json_content: str) -> str:
    """
    Repair common JSON formatting issues from different clients (n8n, claude-code).
    
    Args:
        json_content: Raw JSON content that may have formatting issues
    
    Returns:
        Repaired JSON string
    """
    # Handle n8n specific issues and common JSON formatting problems
    ...
    # Fix mismatched brackets - common n8n issue: ] instead of }
    # Pattern: "arguments": { ... } ] should be "arguments": { ... } }
    import re
    
    # Find pattern where arguments object is followed by ] instead of }
    pattern = r'("arguments"\s*:\s*\{[^}]*\})\s*\](\s*\})'
    match = re.search(pattern, repaired)
    if match:
        # Replace ] with } before the final object closing brace
        repaired = re.sub(pattern, r'\1\2', repaired)
        logger.debug("Fixed mismatched bracket issue (] -> }) in arguments")
    
    # Fix missing commas between array elements
    # Pattern: } { should be }, {
    repaired = re.sub(r'\}\s*\{', '}, {', repaired)
    
    # Fix missing commas after closing braces before array end
    # Pattern: } ] should be } ]
    repaired = re.sub(r'\}\s*\]', '}]', repaired)
    
    # Handle excessive whitespace but preserve structure
...
def parse_tool_calls_from_response(response_text: str) -> List[Dict[str, Any]]:
    ...
    Enhanced to handle malformed JSON from different clients (n8n, claude-code).
    ...
    Raises:
        ValueError: If tool calls cannot be parsed after repair attempts
    """
    tool_calls = []
    ...
    if start_idx != -1 and end_idx != -1 and end_idx > start_idx:
        ...
        # First attempt: parse as-is (for backward compatibility)
        try:
            ...
            logger.debug(f"Initial JSON parsing failed, attempting repair: {e}")
            
            # Second attempt: repair and parse (for malformed JSON from clients)
            try:
                # Skip repair if content is empty or whitespace only
                if not json_content.strip():
                    logger.debug("Empty JSON content, returning empty list")
                    return tool_calls
                
                repaired_json = repair_malformed_json(json_content)
                logger.debug(f"Attempting to parse repaired JSON: {repaired_json[:200]}...")
                
                # Skip parsing if repair resulted in empty content
                if not repaired_json.strip():
                    logger.debug("Repair resulted in empty content, returning empty list")
                    return tool_calls
                
                parsed_calls = json.loads(repaired_json)
                if isinstance(parsed_calls, list):
                    for call in parsed_calls:
                        if isinstance(call, dict) and 'name' in call:
                            tool_calls.append(call)
                            logger.info(f"Successfully parsed repaired tool call: {call.get('name', 'unknown')}")
                        else:
                            logger.warning(f"Invalid tool call format after repair: {call}")
                    return tool_calls
            except json.JSONDecodeError as repair_error:
                # For API compatibility, return empty list instead of raising exception
                logger.warning(f"JSON repair failed, returning empty tool calls list")
                logger.debug(f"Repair error: {repair_error}, original: {e}")
                logger.debug(f"Original content: {json_content}")
                return []
    else:
...
def is_tool_allowed(function_name: str, config: ToolCallingConfig) -> bool:
    """
    Check if a function is allowed to be called with enhanced security configuration.
    ...
    if function_name in config.blocked_functions:
        logger.warning(f"Function '{function_name}' is explicitly blocked")
        return False
    
    # Check if it's a write operation
    if is_write_operation(function_name):
        if config.enable_write_operations:
            # If whitelist exists, function must be in it
            if config.write_operation_whitelist:
                allowed = function_name in config.write_operation_whitelist
                if not allowed:
                    logger.warning(f"Write function '{function_name}' not in whitelist")
                return allowed
            else:
                # Write operations allowed without whitelist restriction
                return True
        else:
            logger.warning(f"Write function '{function_name}' blocked - write operations disabled")
            return False
    ...
    dangerous_patterns = [
        ...
        'file', 'read', 'create', 'modify', 'download',
    ...
    for pattern in dangerous_patterns:
        if pattern in function_name_lower:
            # Check if this dangerous function is explicitly allowed
            if config.allow_dangerous_functions:
                # If specific allowlist exists, function must be in it
                if config.allowed_dangerous_functions:
                    allowed = any(allowed_pattern in function_name_lower 
                                for allowed_pattern in config.allowed_dangerous_functions)
                    if not allowed:
                        logger.warning(f"Dangerous function '{function_name}' not in allowed list")
                    return allowed
                else:
                    # All dangerous functions allowed
                    return True
            else:
...
def is_write_operation(function_name: str) -> bool:
    """
    Check if a function is a write operation.
    
    Args:
        function_name: Name of the function
    
    Returns:
        True if function is a write operation, False otherwise
    """
    write_patterns = ['write', 'edit', 'create', 'modify', 'update', 'save', 'delete', 'remove']
    function_name_lower = function_name.lower()
    return any(pattern in function_name_lower for pattern in write_patterns)
