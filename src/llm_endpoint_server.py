...
from tool_schemas import ToolCallingConfig, load_tool_config_from_file
...
from conversation_manager import conversation_manager  # Import conversation manager
...
def initialize_global_config():
    ...
    try:
        ...
        # Initialize tool calling configuration and handler
        global tool_calling_config, tool_calling_handler
        tool_calling_config = load_tool_config_from_file(config_file)
        tool_calling_handler = ToolCallingHandler(tool_calling_config)
        
        # Log tool configuration for verification
        logger.info(f"Tool configuration loaded - Security profile: {tool_calling_config.security_profile}")
        logger.info(f"Write operations enabled: {tool_calling_config.enable_write_operations}")
        logger.info(f"Dangerous functions allowed: {tool_calling_config.allow_dangerous_functions}")
        if tool_calling_config.write_operation_whitelist:
            logger.info(f"Write whitelist: {tool_calling_config.write_operation_whitelist}")
        if tool_calling_config.allowed_dangerous_functions:
            logger.info(f"Allowed dangerous functions: {tool_calling_config.allowed_dangerous_functions}")
        if tool_calling_config.blocked_functions:
            logger.info(f"Blocked functions: {tool_calling_config.blocked_functions}")
...
def force_token_refresh_optimized():
    ...
    try:
        ...
        try:
            ...
            # OPTIMIZED: Increased buffer from 5 minutes to 15 minutes (3x reduction in file I/O)
            buffer_time = 900 # 15 minutes instead of 5 minutes
...
def get_cached_token_info():
    ...
    with token_cache_lock:
        ...
        # OPTIMIZED: Extended cache validation window from 60 seconds to 300 seconds (5x improvement)
...
def check_token_needs_refresh_optimized():
    ...
    if cached_info:
        ...
        # OPTIMIZED: Extended refresh window to 15 minutes (balanced approach)
        ...
        with token_file_lock:
            ...
            if os.path.exists(token_file):
                try:
                    ...
                    # OPTIMIZED: Extended refresh window to 15 minutes (balanced)
                    if time_until_expiry <= 900: # 15 minutes
...
def ensure_valid_token():
    ...
    if cached_info:
        ...
        # Refresh if token expires within 10 minutes (600 seconds) - less aggressive, safer
        if time_until_expiry <= 600:
            ...
            # Token has more than 10 minutes left - use it
            logger.debug(f"🕐 Cached token valid for {time_until_expiry/60:.1f} more minutes")
            return True
        ...
        with token_file_lock:
            if os.path.exists(token_file):
                try:
                    ...
                    # CONSERVATIVE: Refresh if token expires within 10 minutes (600 seconds) 
                    if time_until_expiry <= 600:
...
def chat_completions():
    ...
    """
    # BULLETPROOF: Initialize ALL variables at the very top to prevent any UnboundLocalError
    safe_prompt_length = 0
    safe_model_used = 'unknown'
    safe_final_prompt = ""
    ...
    try:
        ...
        # Use safe variables from this point forward
        safe_final_prompt = ""
        safe_prompt_length = 0
        safe_model_used = 'unknown'
        ...
        safe_model_used = sf_model  # Update safe variable
        
        # CRITICAL FIX: Build safe_final_prompt BEFORE tool calling branch to ensure it's available for error handling
        # Convert OpenAI messages array to conversation string for error reporting purposes
        system_message = None
        conversation_parts = []
        
        # Build complete conversation context from all messages
        for msg in messages:
            role = msg.get('role')
        ...
        # Populate safe_final_prompt with conversation data for error handling
        if len(conversation_parts) > 0:
            safe_final_prompt = "\n\n".join(conversation_parts)
            safe_prompt_length = len(safe_final_prompt)
        else:
            safe_final_prompt = "No valid messages found"
            safe_prompt_length = len(safe_final_prompt)
        ...
        if tools and tool_calling_handler:
            ...
            # Validate that we have conversation data (already built above)
            if len(conversation_parts) == 0:
                return jsonify({"error": "No messages found"}), 400
            
            logger.info(f"Processing request - Model: {sf_model}, Prompt length: {len(safe_final_prompt)}")
            
            # Calculate appropriate timeout - handled by HTTP client, no signal needed
            timeout = 300 # 5 minutes base timeout
            if len(safe_final_prompt) > 20000:
                timeout = 480 # 8 minutes for large prompts
            elif "claude-4" in sf_model:
                timeout = 420 # 7 minutes for claude-4
            
            logger.info(f"Using HTTP-level timeout: {timeout}s (no signal handler)")
            
            try:
                # Generate response using Salesforce Models API with HTTP timeout
                sf_response = client.generate_text(
                    prompt=safe_final_prompt,
                    model=sf_model,
                    max_tokens=max_tokens,
                    temperature=temperature,
        ...
        # BULLETPROOF: Use only safe variables to prevent scoping issues
        model_used = 'unknown'
        ...
        return jsonify({
            "error": {
                ...
                "details": {
                    ...
                    "prompt_length": safe_prompt_length,
                    "suggestion": _get_timeout_suggestion(safe_prompt_length, model_used)
                }
            }
        }), status_code
    
    except Exception as e:
        logger.error(f"EMERGENCY DEBUG: Exception caught - type: {type(e).__name__}")
        # EMERGENCY DEBUG: Capture exception without referencing potentially undefined variables
        try:
            error_message = str(e)
            # EMERGENCY WORKAROUND: Filter out the persistent UnboundLocalError
            if "cannot access local variable 'prompt_length'" in error_message:
                error_message = "Authentication successful but API request failed"
                logger.error(f"EMERGENCY: Filtered UnboundLocalError, replaced with generic message")
            logger.error(f"EMERGENCY DEBUG: Exception message captured: {error_message}")
        except Exception as string_error:
            error_message = f"Exception occurred but could not convert to string: {type(e).__name__}"
            logger.error(f"EMERGENCY DEBUG: Could not convert exception to string: {string_error}")
        logger.error(f"Error in chat completions: {error_message}")
        
        # BULLETPROOF: Use only safe variables to prevent scoping issues
        model_used = 'unknown'
        logger.error(f"EMERGENCY DEBUG: Variables reinitialized in exception handler - safe_prompt_length={safe_prompt_length}, model_used={model_used}")
        ...
        try:
            # Aggressive fallback strategy for model selection
            if 'sf_model' in locals():
                model_used = sf_model
            elif data and isinstance(data, dict):  # Added additional type checking
                model_used = data.get('model', 
                    data.get('messages', [{}])[0].get('model', 'unknown')
                )
        except Exception as length_error:
            logger.warning(f"Fallback model calculation error: {length_error}")
        ...
        return jsonify({
            "error": {
                ...
                "details": {
                    "model_used": model_used,
                    "prompt_length": safe_prompt_length,
                    "suggestion": _get_error_suggestion(error_message, safe_prompt_length, model_used)
                }
            }
        }), status_code
...
def _get_error_suggestion(error_message: str, safe_prompt_length: int, model_used: str) -> str:
...
def anthropic_messages():
    ...
    try:
        ...
        # Convert OpenAI messages array to Salesforce-compatible format
        system_msg = None
        conversation_parts = []
...
@app.route('/v1/conversations', methods=['GET', 'POST', 'DELETE'])
def conversations():
    """
    Manage conversations with create, retrieve, and delete capabilities.
    
    GET: Retrieve conversation history
    POST: Create a new conversation or add messages
    ...
    try:
        if request.method == 'GET':
            ...
            # Create new conversation or add messages
            data = request.get_json()
            ...
            if 'create' in data and data['create']:
                ...
                return jsonify({
                    "conversation_id": conversation_id,
                    "status": "message_added" if success else "failed"
                })
